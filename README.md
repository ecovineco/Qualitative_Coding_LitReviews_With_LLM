...
### 5.1 — Data Governance and GDPR Compliance
 
The EU General Data Protection Regulation (GDPR, Regulation (EU) 2016/679, in force since 25 May 2018) governs any personal data in a corpus processed by this pipeline. It applies to the processing of EU/EEA residents' personal data regardless of where the processing organisation itself is based, and its consistent application is coordinated by the European Data Protection Board (EDPB) across the national data protection authorities of each member state. For background on the regulation itself and the authorities that enforce it, see the European Commission's overview of the EU data protection legal framework (https://commission.europa.eu/law/law-topic/data-protection/data-protection-eu_en) and the EDPB (https://www.edpb.europa.eu/). Most literature-review corpora — published papers, policy documents, grey literature — contain no personal data at all, but where a corpus does (unpublished submissions, interview transcripts, case files, and the like), the considerations below determine which provider is the more defensible choice, and are the basis for the recommendation at the end of this section.
 
**Azure OpenAI.** Microsoft's EU Data Boundary is a standing commitment, rolled out in phases since 2023, to store and process Customer Data — and, since its second phase, pseudonymized personal data from system-generated logs — inside the EU/EFTA (https://www.microsoft.com/en/trust-center/privacy/european-data-boundary-eudb). This is region-dependent rather than automatic: an Azure service is only in-scope once it is actually deployed in an EU/EFTA region, so `AZURE_OPENAI_ENDPOINT` in `config.py` needs to point at an EU/EFTA deployment for the Boundary commitment to apply to this pipeline's traffic (https://learn.microsoft.com/en-us/privacy/eudb/eu-data-boundary-learn). Microsoft documents a limited, named set of exceptions where data still leaves the boundary regardless of region — global security-incident response, worldwide service-quality monitoring, and fulfilling a GDPR data-subject request that spans regions — each covered by its own contractual safeguards (https://learn.microsoft.com/en-us/privacy/eudb/eu-data-boundary-transfers-for-all-services). Separately from data residency, Microsoft publishes a four-step (discover, manage, protect, report) framework and Azure-specific tooling — Azure Policy, Microsoft Purview, the Data Subject Request portal, Compliance Manager — for meeting GDPR's substantive obligations (https://download.microsoft.com/documents/en-gb/Microsoft-Azure-and-the-EU-GDPR.pdf), and a fuller account of the Boundary's rollout and history is in Microsoft's own recap (https://www.microsoft.com/content/dam/microsoft/final/en-us/microsoft-brand/documents/Understanding-the-EU-Data-Boundary-External-01-12-2024.pdf) and original 2021 announcement (https://blogs.microsoft.com/eupolicy/2021/05/06/eu-data-boundary/).
 
**Anthropic.** Use of the Anthropic API — the path this pipeline uses — is governed by Anthropic's Commercial Terms of Service, which automatically incorporate a Data Processing Addendum (DPA) built on the EU's Standard Contractual Clauses (SCCs): accepting the Terms means accepting the DPA, with no separate signature needed (https://www.anthropic.com/legal/commercial-terms, https://privacy.anthropic.com/en/articles/7996862-how-do-i-view-and-sign-your-data-processing-addendum-dpa). Anthropic is contractually barred from training its models on data submitted through the API (https://www.anthropic.com/legal/commercial-terms), and documents SOC 2 Type II and ISO 27001/ISO 42001 certifications, its sub-processor list, and further compliance material on its Trust Center and Privacy Center (https://trust.anthropic.com, https://privacy.anthropic.com/). The material difference from Azure is data residency rather than compliance posture: Anthropic's Privacy Policy states that data is transferred to, and processed on, its servers in the US or other countries outside the EEA/UK, with GDPR-compliant transfer relying on an adequacy decision or SCCs rather than on keeping the data inside the EU (https://www.anthropic.com/legal/privacy).
 
**In short**, both providers satisfy the baseline GDPR requirements for a processor — a DPA, a lawful international-transfer mechanism, and recognised security certifications — but of the two, only Azure OpenAI, deployed in an EU/EFTA region, keeps the data inside the EU rather than transferring it out under contractual safeguards. For a corpus that is already public, which covers most literature reviews, that distinction rarely matters in practice. For a corpus containing personal, confidential, or otherwise sensitive or regulated material, **Azure OpenAI (deployed in an EU/EFTA region) is the appropriate default**; the Anthropic path remains a good and fully GDPR-compliant choice for everything else. None of this is legal advice — an organisation's own data protection officer or legal counsel should confirm that either provider's contractual terms satisfy its specific obligations before sensitive material is processed.
